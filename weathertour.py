...
if __name__ == '__main__':
    ...
    parser.add_argument('--start', default=datetime.datetime.now().astimezone(), type=pyrfc3339.parse)
    parser.add_argument('--speed', default=60, type=int)
    ...
    if gpx.routes:
        for route in gpx.routes:
            ...
            follow_tour(route.points, args.start, args.speed)
    else:
        for track in gpx.tracks:
            for segment in track.segments:
                logger.info('Start segment')
                follow_tour(segment.points, args.start, args.speed)
